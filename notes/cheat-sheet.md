...
- 🧠 **OutsideAWS + can't stop instance: TokenIssueTime (stop attacker) + no-reboot AMI (memory) + EBS snapshot (disk) + IMDSv2 hop limit 1 (prevent future SSRF). Deny-all SG kills legitimate traffic — wrong choice if API must stay up.**
