...
═══ COGNITO + DYNAMODB ABAC (sub vs TenantId) ═══
  ...
  Per-USER DynamoDB access:
    ...
    → Each user accesses ONLY their own rows
    → "sub" = unique per user (auto-generated by Cognito)

  Per-TENANT DynamoDB access:
    → Map custom:TenantId claim → session tag
    → dynamodb:LeadingKeys = ${aws:PrincipalTag/TenantId}
    → All users in same tenant share rows
    → "sub" ≠ TenantId (sub is per-user, TenantId is per-org)

  TRAP: "sub" for per-user. Custom claim for per-tenant. Don't mix.

═══ BOUNDARY vs SERVICE CATALOG (failed Q1235) ═══

  "Teams independently CREATE roles within guardrails"
    = Permission boundary delegation (SCP forces boundary on CreateRole)
    = One-time setup, teams create ANYTHING within ceiling

  "Teams SELECT from pre-approved menu, no broad IAM"
    = Service Catalog (launch constraint)
    = Security team builds templates, devs just pick and launch

  RULE: "Independently provision" = boundary. "Self-service deploy" = SC.

═══ YOUR ERROR PATTERNS ═══

  ❌ Confused Cognito with Identity Center (Q778)
     → RULE: Customers = Cognito. Employees = Identity Center. Never mix.

  ❌ Called AssumeRoleWithWebIdentity directly (Q778)
     → RULE: Use Identity Pool (managed STS). Don't call STS yourself.

  ❌ Picked wrong AD service (Q709, Q731, Q739)
     → RULE: "Need trusts" or "RDS SQL" = Managed AD. Always.
       "No AWS infra" = ADFS + Identity Center (not AD Connector).
       "Proxy only" = AD Connector. "Standalone" = Simple AD.

  ❌ Picked Service Catalog for "independently create roles" (Q1235)
     → RULE: SC = pick from menu. Boundary = create within ceiling.

  ❌ Used Bool instead of BoolIfExists for MFA
     → RULE: CLI/SDK has no MFA key → Bool skips → hole.
       BoolIfExists = treats absent as match → Deny fires. Always.
```
